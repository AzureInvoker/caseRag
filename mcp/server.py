...
TOOLS = [
    ...
    },
    {
        ...
        "description": "【重要】添加前先调用 tc_project_types 查询已有项目类型。添加单条测试用例。"
            "自动清洗规则：标题/模块必填非空；project 为项目类型（如 slot游戏/后台/活动），空则留空；priority→P3；category→功能测试；creator→MCP；steps/tags 空元素自动过滤；所有文本 trim+合并换行空白。非字符串字段降级为空串。"
    ...
    {
        "name": "tc_add_batch",
        "description": "【重要】添加前先调用 tc_project_types 查询已有项目类型。批量添加测试用例（逐条清洗，单条失败不阻塞整体）。清洗规则同 tc_add：标题/模块必填非空；project 为项目类型，空则留空；priority→P3；category→功能测试；creator→MCP；steps/tags 空元素自动过滤；所有文本 trim+合并空白。"
            "返回成功/失败统计和模块分布。"
            "⚠️ 添加后自动同步写入 LightRAG 知识图谱（如已启用），后续可用 tc_graph_search 检索新用例的实体关系",
        "inputSchema": {
            "type": "object",
...
def handle_tool(name: str, args: dict) -> dict:
    if name == "tc_search":
        ...
        if not mcp_lightrag.is_available():
            return {"content": [{"type": "text", "text": "❌ LightRAG 图谱未启用或初始化失败。可调 tc_graph_status 查看详情"}]}
        query = args.get("query", "").strip()
        if not query:
            return {"content": [{"type": "text", "text": "请提供搜索关键词"}]}
        result = mcp_lightrag.search(query, n_results=min(int(args.get("n_results", 5)), 20))
        ...
        status = mcp_lightrag.get_status()
        text = f"## LightRAG 状态\n启用: {status.get('enabled')}\n就绪: {status.get('ready')}\n提供商: {status.get('provider', '-')}\n模型: {status.get('model', '-')}\n"
        if status.get("node_count") is not None:
            text += f"实体数: {status['node_count']}\n"
        return {"content": [{"type": "text", "text": text.strip()}]}
        ...
        if method == "initialize":
            send_json({
                ...
                "result": {
                    ...
                    "capabilities": {"tools": {}},
                    "serverInfo": {"name": "testcase-rag-mcp", "version": "3.0.0"},
                },
            })
